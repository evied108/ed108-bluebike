...
plot(result)
```
The plot above suggests that the lowest deviance occurs when there are three terminal nodes used in the tree. We can realize such a tree by using the function prune.tree:
...
Having made both models, we can compare their performances against each other using our testing set.
``` {r predicts}
...
Attempting random forest plot:
``` {r random forest}
set.seed(123)
fit.RF <- randomForest(bluebike_riders~.,data=data.train[,-c(1)],ntree=100,mtry=4, importance=TRUE)
Yhat.RF <- predict(fit.RF,newdata=data.test[,-c(1)],type="class")

#misclassification rate
sum(Yhat.RF!=data.test[,6])/18
